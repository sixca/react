...
async function mysqlRead(sql, params) //SQL 쿼리문과 해당 쿼리문에 사용될 매개변수 배열 params를 받음
{
    let promise = new Promise((resolve, reject)=>{
        readpool.getConnection( (err, conn)=>{ //DB풀과 연결. conn은 연결 객체로 이름 지어줌
            ...
            conn.query(sql, params, (err, rows)=>{ //conn.query 메서드를 사용하여 SQL쿼리문을 실행하고 결과를 반환. 결과는 프로미스로 저장
                console.log(sql);
                console.log(rows);
                if(err)
                    reject(err); //에러 시 reject 메서드로 프로미스를 거부
                else
                    resolve(rows); // rows가 들어오면(에러가 없으면) resolve 메서드가 프로미스를 이행함
                conn.release(); //release 메서드가 연결을 풀에 반환. 즉, 종료가 아니라 연결을 사용한 후 해당 연결을 필요할 때마다 가져와 사용하는 것. 다른 요청이 일어나면 또 재사용함.(성능향상) 마무리 메서드이긴 함
            })
